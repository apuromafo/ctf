...
ls -l ~/.ssh
Check for SSH keys for
current user
history
Check the current user's Bash
history
sudo -l
Can the user run anything as
another user?
ls -la /etc/cron.daily
Check for daily Cron jobs
lsblk
Check for unmounted file
systems/drives
find / -path /proc -prune -o -type d -perm -o+w
2>/dev/null
Find world-writeable
directories
find / -path /proc -prune -o -type f -perm -o+w
2>/dev/null
...
\; 2>/dev/null
Find binaries with the SUID
bit set
find / -user root -perm -6000 -exec ls -ldb {}
\; 2>/dev/null
Find binaries with the
SETGID bit set
sudo /usr/sbin/tcpdump -ln -i ens192 -w
...
find / ! -path "*/proc/*" -iname "*config*" -
type f 2>/dev/null
Search for config files
ldd /bin/ls
View the shared objects
...
Start a privileged LXD
container
lxc config device add r00t mydev disk source=/
path=/mnt/root recursive=true
Mount the host file system in
a container
lxc start r00t
Start the container
...
Created a shared tmux
session socket
./lynis audit system
Perform a system audit with
Lynis