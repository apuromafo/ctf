# 📋 Scattered Spider [MEDIUM]

![Thumbnail](thumbnail.png)

## 📈 Resumen Ejecutivo / Executive Summary
- **Industria / Industry:** Telecommunications
- **Tipo de Ataque / Attack Type:** Data theft
- **Arquitectura / Architecture:** Cloud
- **Sistemas Operativos / OS:** Linux, Mac

## 📝 Contexto del Incidente / Incident Context
Customer support escalates a surge in complaints about suspicious account activity. Internal review reveals that an administrator account has been used to extract large volumes of customer data.

## 🎯 Objetivos de la Simulación / Simulation Objectives
- Increase Organizational Awareness and Preparedness
- Enhance Team Coordination and Communication
- Test and Validate Incident Response Plans and Procedures

## 🛠️ Aplicaciones y Herramientas Involucradas / Applications & Tools Involved
- Mobile apps
- Web apps
- Email platform

---
*Escenario creado por / Scenario created by: THMDan*